...
# from config import _sse, is_windows, parse_report_file, load_batch_config, load_analysis_config


# def register_routes(app, ctx: dict, pipeline_fns: dict):
...
#     _run_averager_pipeline = pipeline_fns["run_averager"]
#     _run_analysis_plots    = pipeline_fns["run_plots"]
...
#         fn         = data.get("settings_filename", f"settings_{int(time.time()*1000)}.txt")
...
#         def generate():
#             if do_compile:
#                 yield _sse({"type": "log", "level": "info", "message": "Compiling ONE…"})
#                 compile_script = SIMULATOR_DIR / ("compile.bat" if is_windows() else "compile.sh")
#                 if not compile_script.exists():
#                     yield _sse({"type": "log", "level": "warning", "message": "compile script not found, skipping."})
#                 else:
#                     r = subprocess.run(
...
#                         yield _sse({"type": "complete", "success": False, "message": "Compile failed"})
#                         return
#                     yield _sse({"type": "log", "level": "success", "message": "Compile OK"})

#             yield _sse({"type": "log", "level": "info", "message": f"Starting ONE — {n} run(s) — {fn}"})
...
#                     line = line.rstrip()
#                     if line:
#                         yield _sse({"type": "log", "level": "output", "message": line})

#                 proc.wait()
#                 with _lock: _processes.pop(str(proc.pid), None)

#                 if proc.returncode != 0:
#                     yield _sse({"type": "complete", "success": False, "message": f"ONE exited with code {proc.returncode}"})
#                     return

#             except Exception as exc:
#                 yield _sse({"type": "complete", "success": False, "message": str(exc)})
#                 return

#             yield _sse({"type": "complete", "success": True, "message": f"Simulation done — {n} run(s)"})
...
#         reports_dir = (SIMULATOR_DIR / body.get("batchFolder", "reports/")).resolve()

#         if not reports_dir.exists():
#             return jsonify({"success": False, "message": f"Reports folder not found: {reports_dir}"}), 404

#         batch_cfg = _load_batch()
#         fp = batch_cfg.setdefault("filename_pattern", {})
#         if body.get("batchDelimiter"): fp["delimiter"] = body["batchDelimiter"]
#         if body.get("batchExtension"): batch_cfg["file_extension"] = body["batchExtension"]
#         if body.get("batchPrecision"): batch_cfg.setdefault("output", {})["precision"] = int(body["batchPrecision"])
...
#         body        = request.json or {}
#         reports_dir = (SIMULATOR_DIR / body.get("batchFolder",      "reports/")).resolve()
#         plots_base  = (SIMULATOR_DIR / body.get("analysisPlotsDir", "plots/")).resolve()

#         if not reports_dir.exists():
#             return jsonify({"success": False, "message": f"Reports folder not found: {reports_dir}"}), 404

#         analysis_cfg = _load_analysis()
#         avg = analysis_cfg.setdefault("filename_structure", {}).setdefault("average_files", {})
#         if body.get("analysisReportTypePos")   is not None: avg["report_type_position"]   = int(body["analysisReportTypePos"])
#         if body.get("analysisRouterPos")        is not None: avg["router_position"]        = int(body["analysisRouterPos"])
#         if body.get("analysisGroupingTypePos")  is not None: avg["grouping_type_position"] = int(body["analysisGroupingTypePos"])
#         if body.get("enabled_plots"): analysis_cfg["enabled_plots"].update(body["enabled_plots"])
