...
export function buildSettingsContent(s) {
  ...
  out += `Group.nrofHosts = ${s.commonGroup.nrofHosts}\n\n`;

  // Per-group output
  s.groups.forEach((g, i) => {
    const n = i + 1;
    const appType = g.applicationType || "";

    out += `Group${n}.groupID = ${g.groupID}\n`;
    out += `Group${n}.nrofHosts = ${g.numHosts}\n`;
    out += `Group${n}.movementModel = ${g.movementModel}\n`;

    // Route file / type (MapRouteMovement)
    ...
    out += `Group${n}.messageTTL = ${g.messageTTL}\n`;

    if (g.activeTimes && g.activeTimes !== "0")
      out += `Group${n}.activeTimes = ${g.activeTimes}\n`;

    // Application
    out += `Group${n}.nrofApplications = 1\n`;
    if (appType) out += `Group${n}.application1 = ${appType}\n`;

    // Interface
    const iface = g.interface1 || "btInterface";
    out += `Group${n}.nrofInterfaces = 1\n`;
    out += `Group${n}.interface1 = ${iface}\n`;
